check_mas()
{
	...
	if test "$char1" = "M"; then
		test "$char2" = "S"
	elif test "$char1" = "S"; then
		test "$char2" = "M"
	else
		return 1
	fi
...
for ((row_idx = 1; row_idx < row_nr - 1; row_idx++)); do
	row="${input[row_idx]}"
	for ((col_idx = 1; col_idx < col_nr - 1; col_idx++)); do
		if test "${row:col_idx:1}" = "A"; then
